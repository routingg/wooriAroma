  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-10">
      ...
      <div className="flex flex-wrap gap-2">
        <StatusFilterPill href="/admin/reservations" label="전체" selected={!statusFilter} />
        {STATUS_OPTIONS.map((s) => (
          <StatusFilterPill
            key={s}
            href={`/admin/reservations?status=${s}`}
            label={STATUS_LABELS_KO[s]}
...
function StatusFilterPill({ href, label, selected }: { href: string; label: string; selected: boolean }) {
  return (
    <Link
      href={href}
